        ...
        for os_n in range(len(oscmd_list)):
            ...
            if(response1.elapsed.total_seconds()-response.elapsed.total_seconds()>15):
                ...
                for line in log:
                    ...
                    # log_payloadの設定
                    log_list[12] = log_list[12].replace('+',' ')
                ...
                explanation = "発生しうる脅威：重要情報の漏えい、不正なシステム操作、ウイルスなどへの感染、他システムへの攻撃の踏み台等\n解決法：IPA 安全なウェブサイトの作り方{https://www.ipa.go.jp/files/000017316.pdf}[2-(i)]等"
